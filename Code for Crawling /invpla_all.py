...
def franglinks(keywords,statyear,endyear):
	...
	for i in range(10000):
		...
		# Find pages
		time_data = soup.findAll('p',{'class':'search-meta'})
		if time_data:
...
def invpla(keywords):
	...
	fart = open('invplaart_all.csv','ab+')
	...
	for link in links:
		...
		tx_data = soup.find_all('div',{'class':'entry-content'})
		oneart = []
		for item in tx_data:
		...
		tm_data = soup.find_all('span',{'class':'entry-date'})
		for item in tm_data:
			try:
