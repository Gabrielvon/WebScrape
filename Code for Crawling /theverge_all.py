...
def findpage(statdate,enddate,time_data):
	...
	for tm_data in time_data:
		for item in tm_data.find_all('p',{"class": "byline"}):
			datetext = str(item.text).strip() #the text include date and time
			# temp = re.search(statdate,datetext).group()
...
def franglinks(keywords,statyear,endyear):
	...
	for i in range(10000):
		...
		# Find pages
		time_data = soup.find_all('div',{'class':'m-search-page__content'})
		if time_data:
...
########---------- Crawling(Main Part) ----------########
def theverge(keywords):
	ii = 0
	links = franglinks(keywords,2015,2014)
	fart = open('thevergeart_all.csv','ab+')
