...
def findpage(statdate,enddate,time_data):
	...
	for item in time_data:
		for titem in item.find_all('time'):
			datetext = str(titem.text).strip() #the text include date and time
...
def franglinks(keywords,statyear,endyear):
	...
	for i in range(10000):
		...
		# Find pages
		time_data = soup.find_all('div',{'class':'search-results'})
		if time_data:
...
def wired(keywords):
	...
	for link in links:
		...
		tx_data = soup.find_all('section',{'tabindex':'0'})
		oneart = []
		for item in tx_data:
